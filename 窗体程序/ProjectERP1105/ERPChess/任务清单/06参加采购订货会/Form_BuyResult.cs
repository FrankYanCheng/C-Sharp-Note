...
namespace ERPChess
{
    public partial class Form_BuyResult : LSGOfrmBasement
    {
        ...
        {
            if (Program.Forms.frmMain.lbl_Paccount4.InvokeRequired)
            {
                paRefresh r = paccountRefresh;
                Program.Forms.frmMain.lbl_Paccount4.Invoke(r, d);
            }
            else
            {
                Program.Forms.frmMain.lbl_Paccount4.Text = d.ToString()+"百万";
                Program.Forms.frmMain.lbl_Paccount4.Refresh();
            }
        }
        delegate void step();
